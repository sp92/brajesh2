...
<!-- banner -->
	<div class="banner">
	 ...
	 mysql_query("update products set prod_hits = (prod_hits+1) where prod_id=$pid");
	 $sql = "select * from products p
				join category c on  p.prod_cat = c.cat_id
				left join subcategory sc on  sc.cat_id= c.cat_id and p.prod_subcat = sc.subcat_id 
				where prod_id=$pid and prod_active=1";
	 ...
	 {					 	
		 ...
		 $wid= $row["writer"];
		 $cat = $row["prod_cat"];		
		 $pic = getpic("prod_pic/large/". trim($row["prod_pic"]));		
		 ...
		 $disp = "inline";
		 if ($row["prod_store"] != $StoreId)
			 $disp = "none";
		 
		// if ($row["prod_cat"] == 133)
  		//	$extra_link = 'http://www.manojpublications.com';
...
?>
		...
		<div class="w3l_banner_nav_right">
			...
			<div class="agileinfo_single">
				<h5><? echo trim($row["prod_name_e"]);?></h5>
				<div class="col-md-4 agileinfo_single_left">
					<img id="example" src="<? echo $pic;?>" alt=" " class="img-responsive" />
				</div>
				<div class="col-md-8 agileinfo_single_right">
					...
					<div class="w3agile_description">
						 ...
						 <? if ( $row["extra"] != "") {
                            $pid = getValue("select prod_id from products where prod_code='".$row["extra"]."'");
                            $link_text = "This book also available in Hindi. Click here to see more...";
                          if ($row["prod_type"]=="H")
                            $link_text = "This book also available in English. Click here to see more...";
                         ?>
                         <p><a href="details.php?prod_id=<? echo $pid ?>" ><strong><? echo $link_text ?></strong></a></p>
                         <? } ?>
                         <? if ($row["prod_code"]<> "" ) { ?>
                            <p><b>Code:</b> <? echo $row["prod_code"]?></p>
                            <? } ?>
                         <? if ($row["prod_type"]<> "" ) { ?>
                            <p><b>Type:</b> <? echo $row["prod_type"]?></p>
                            <? } ?>
                           <? if ($row["pages"]<> "" ) { ?>
                            <p><b>Pages:</b> <? echo $row["pages"]?></p>
                            <? } ?>
                             <? if ($row["prod_format"]<> "" ) { ?>
                            <p><b>Format:</b> <? echo $row["prod_format"]?></p>
                            <? } ?>
                            <? if ($row["prod_isbn"]<> "" ) { ?>
                            <p><b>ISBN:</b> <? echo $row["prod_isbn"]?></p>
                            <? } ?>
                            <? if ($row["prod_desc"]<> "" ) { ?>
                            <p><b>ISBN:</b> <? echo $row["prod_desc"]?></p>
                            <? } ?>
					</div>
					<div class="snipcart-item block">
						<div class="snipcart-thumb agileinfo_single_right_snipcart">
						 <? if ($Discount>0){ ?>
							<h4><i class="fa fa-inr"></i> . <? echo( $SalePrice ) ?><span><i class="fa fa-inr"></i> . <? echo ($PrintMRP) ?></span>
							&nbsp;&nbsp;&nbsp;<? echo ($Discount) ?>% off 
                                            <? } else { ?>
                                             <i class="fa fa-inr"></i> . <? echo ($PrintMRP) ?>/- 
                                             <? } ?></h4>
						</div>
							...
							<form action="#" method="post">
								<fieldset>
									...
									<input type="hidden" name="cancel_return" value=" " />-->
									 <? if (($SalePrice>0 && $PrintMRP > 0 && ($row["stock"] == 'In Stock' || $row["stock"] == 'In Stock' )) || $Discount == 100) { ?>
									
				<a class="button blue-btn btn butS mr10 " style="cursor:pointer; border:1px solid;" id=<? echo $row["prod_id"] ?> onClick="fn_addcart(<? echo $row["prod_id"] ?>,$('#qty').val());">  Add to Cart <i class="fa fa-shopping-cart white-color"></i></a>
									<a href="javascript:fn_orderNow(<? echo $row["prod_id"] ?>,$('#qty').val())" style="cursor:pointer; border:1px solid;" class="button blue-btn btn butS mr10 "><? echo $BtnText ?><i class="fa fa-cart-plus"></i></a>
